...
unordered_map<string, int> deserializeMap(const string& data)
{
    ...
    while (getline(ss, line))
    {
        ...
        try
        {
            ...
            if (!word.empty()) frequencies[word] += count;
        }
        catch (...) {}
    }
    return frequencies;
...
void printTopWords(const unordered_map<string, int>& frequencies, int limit = 10)
{
    ...
    cout << "Unique words: " << frequencies.size() << "\n";
}
...
int main(int argc, char* argv[])
{
    ...
    if (rank == 0)
    {
        if (argc < 2)
        {
            cerr << "Usage: mpirun -np <N> " << argv[0] << " file1.txt file2.txt ..." << endl;
    ...
    if (rank == 0)
    {
        ...
        double singleTime = chrono::duration_cast<chrono::microseconds>(singleEnd - singleStart).count() / 1000.0;

        cout << "=== Single-thread ===\n";
        cout << "Execution time: " << singleTime << " ms\n\n";

        auto ompStart = chrono::high_resolution_clock::now();
        auto ompResult = openMpWordCount(files, size);
        auto ompEnd = chrono::high_resolution_clock::now();
        double ompTime = chrono::duration_cast<chrono::microseconds>(ompEnd - ompStart).count() / 1000.0;

        cout << "=== OpenMP (threads: " << size << ") ===\n";
        cout << "Execution time: " << ompTime << " ms\n";
    ...
    MPI_Gather(&localSize, 1, MPI_INT, receiveSizes.data(), 1, MPI_INT, 0, MPI_COMM_WORLD);
    ...
    if (mapSendBuf.empty()) mapSendBuf.push_back(0);
    ...
    if (rank == 0)
    {
        ...
        for (int i = 0; i < size; i++)
        {
            if (receiveSizes[i] == 0) continue;
            string part(receiveBuffer.data() + recvDisplacements[i], receiveSizes[i]);
            auto tempMap = deserializeMap(part);
            mergeMaps(globalMpiFrequencies, tempMap);
        }

        cout << "=== MPI (processes: " << size << ") ===\n";
        cout << "Execution time: " << (mpiEnd - mpiStart) * 1000.0 << " ms\n";
